...
], function (Controller, JSONModel, BusyIndicator,DomainFallback) {
  "use strict";
    ...
    onInit: function () {
      ...
      oModel.read(sPath, {
        ...
        success: function (oData) {
          ...
          // Applica val1..val5 SOLO per campi MultipleVal='X' che:
          // - hanno Dominio vuoto, oppure
          // - hanno Dominio non presente in domainsByName, oppure
          // - hanno Dominio presente ma senza values
          (function applyDomainFallbackIfNeeded() {
            ...
            try {
              if (DomainFallback && typeof DomainFallback.isEnabled === "function") {
                ...
                // fallback minimo se util non ha isEnabled
            ...
            // fallback INLINE (val1..val5) se util non ha apply()
            ...
            var aPatched = aAllFields.map(function (f) {
              ...
              // serve mock (dominio vuoto / dominio mancante / dominio senza valori)
          ...
          // mmctFieldsByCat: Cat -> raw fields[]  (ricostruito dai fields patchati)
          ...
          // preserva coerenza anche dentro aMMCT (se qualcuno usa cat.UserMMCTFields.results)
