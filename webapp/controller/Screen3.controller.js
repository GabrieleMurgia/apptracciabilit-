  ...
  return Controller.extend("apptracciabilita.apptracciabilita.controller.Screen3", {
    ...
    _buildRecords01: function (aAllRows) {
      ...
      (aAllRows || []).forEach(function (r) {
        ...
        var sStatus = this._getRowStatus(r);
        var bApproved = this._isApproved(sStatus);

        var rec = {
          idx: a.length,
          guidKey: sGuidKey,
          Fibra: sFibra,
          Stato: sStatus,          // utile se vuoi mostrarlo
          __status: sStatus,
          __approved: bApproved
        };
    ...
    // =========================
    // P13N force visible (se ti nasconde colonne)
    // =========================
    ...
    _rebuildColumnsHard: async function (oTbl, aCfg01) {
      ...
      // ✅ editabile SOLO se __approved = false
