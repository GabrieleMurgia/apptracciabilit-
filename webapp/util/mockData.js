...
sap.ui.define([], function () {
  ...
  function applyVm(oVm, cfg) {
    ...
    // =========================================================
  // MOCK DA FILE JSON (risposta reale salvata in webapp/mock/*.json)
  // =========================================================
  function _resolveAppUrl(relPath) {
    var p = String(relPath || "").replace(/^\//, "");
    // base: apptracciabilita/apptracciabilita
  ...
  function _unwrapODataPayload(x) {
    ...
    // OData V2 tipico: { d: {...} }
    if (x.d) return x.d;
    // OData V4 tipico: { value: [...] }
    if (x.value) return x.value;
    return x;
  }

  function _asResultsArray(x) {
    // supporta: {results:[...]} oppure array diretto
    if (!x) return [];
  ...
  function applyVmFromUserInfosPayload(oVm, oData, cfg) {
    ...
    var userType = String(cfg.userType || oData.UserType || "").trim().toUpperCase(); // E/I/S
    ...
    // preserva coerenza anche dentro aMMCT (come fai in Screen0)
    ...
    // payload compatibile col tuo vm (come in Screen0)
    var payload = {
      userId: userId,
      userType: userType,
      ...
      // preserva flags mock già settati
      mock: oVm.getProperty("/mock") || {}
    };
  ...
  function applyVmFromFile(oVm, cfg) {
    ...
    return fetch(url, { cache: "no-store" }).then(function (resp) {
      ...
      return applyVmFromUserInfosPayload(oVm, json, cfg);
    });
  }
